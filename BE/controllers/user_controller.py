...
def login():
    try:
        ...
        if jwtTokens:
            ...
            response.headers['Set-Cookie'] = f'Authorization={authorize}; Expires={expiration_authorization.strftime("%a, %d %b %Y %H:%M:%S GMT")}; HttpOnly; SameSite=None; Max-Age={int((expiration_authorization - datetime.utcnow()).total_seconds())}; Path=/; Secure; Refresh={refresh}; Expires={expiration_refresh.strftime("%a, %d %b %Y %H:%M:%S GMT")}; SameSite=None; Max-Age={int((expiration_refresh - datetime.utcnow()).total_seconds())}; Path=/; Secure;'
