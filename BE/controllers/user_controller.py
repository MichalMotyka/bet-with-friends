...
def register():
    ...
    try:
        ...
        response = make_response(Response(message='Pomyślnie utworzono użytkownika',code='OK',time_stamp=datetime.utcnow().timestamp()).__dict__)
        ...
        return make_response(Response(message=e.message,code='R1',time_stamp=datetime.utcnow().timestamp()).__dict__)
    except User_Alredy_Exist_Name_Exception as e1:
        response = make_response(Response(message=e1.message,code='R2',time_stamp=datetime.utcnow().timestamp()).__dict__)
        response.status_code = 400
...
def activate(code:str):
    ...
    return make_response(Response(message='Pomyślnie aktywowano użytkownika',code='OK',time_stamp=datetime.utcnow().timestamp()).__dict__)
...
def login():
    ...
    if jwtTokens:
        ...
        response = make_response(Response(message='Pomyślnie zalogowano użytkownika',code='OK',time_stamp=datetime.utcnow().timestamp()).__dict__)
        ...
        return response
    return make_response(Response(message='Użytkownik nie posiada konta lub hasło jest nieprawidłowe',code='OK',time_stamp=datetime.utcnow().timestamp()).__dict__)
