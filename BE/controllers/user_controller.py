...
def login():
    try:
        ...
        if jwtTokens:
            ...
            response.headers['Set-Cookie'] = f'Refresh={refresh}; Expires={expiration_refresh.strftime("%a, %d %b %Y %H:%M:%S GMT")}; Max-Age={int((expiration_refresh - datetime.utcnow()).total_seconds())}; Path=/;'
