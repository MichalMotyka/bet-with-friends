    ...
    with session_factory() as session:
       posible_best =  (session
               ...
               .filter(Competition.public_id == competetition, Match.utc_date > datetime.utcnow(), 
                       Match.utc_date <= datetime.now() + timedelta(days=6)
                       ,Bets.match_id == None)
               .order_by(Match.utc_date)
               .offset((page-1)*limit)
               .limit(limit)
       ...
       count = (session
               ...
               .join(Competition)
               .outerjoin(Bets, Match.id == Bets.match_id)
               .filter(Competition.public_id == competetition, Match.utc_date <= datetime.now() + timedelta(days=6), Match.utc_date > datetime.utcnow(),Bets.match_id == None)
               .count())
